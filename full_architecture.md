...
flowchart TB
    subgraph Users["Users"]
        direction LR
        Prospect["Prospect<br/>web · SMS · WhatsApp · email · FB Messenger · voice"]
        Manager["Manager<br/>(browser)"]
    end

    subgraph Agent["Agent Backend — App Service 03 (FastAPI)"]
        direction TB
        Channels["Channel Adapters<br/>signature validation · outbound senders"]
        Entry["Unified Entry<br/>handle_chat_message()<br/>tenant resolution · identity merge · history load"]
        Loop["Agent Loop<br/>Anthropic Haiku 4.5 + extended thinking<br/>10-tool catalog · 4 response validators"]
        Retrieval["Two-Stage Retrieval<br/>FAISS Stage 1 (per-tenant, per-KB)<br/>gpt-4o-mini Stage 2 (cited candidates)"]
        Async["Background Jobs<br/>APScheduler: tickets 60s · follow-ups 15m<br/>Email polling ~20s"]
        MgrAPI["/manager/* REST API<br/>+ SSE event stream"]
    ...
    subgraph Web["KVR Platform — App Service 04"]
        direction TB
        SPA["React 18 + Vite SPA<br/>(Azure Static Web App)<br/>JWT bearer auth"]
        BE["FastAPI Backend<br/>103 REST routes · 53 ORM classes<br/>SQLAlchemy 2.0"]
        SPA --> BE
    end

    DB[("Azure SQL Server 2022<br/>97 tables · 11 schemas<br/>shared persistence layer")]

    subgraph Ext["External Services"]
        direction TB
        Anthropic["Anthropic API<br/>(primary LLM)"]
        OpenAI["OpenAI API<br/>(Stage 2 + nano utility)"]
        Tavily["Tavily<br/>(web search fallback)"]
        Twilio["Twilio<br/>(SMS · WhatsApp · voice)"]
        MSGraph["Microsoft Graph<br/>(email send + poll)"]
        FBGraph["Facebook Graph<br/>(Messenger)"]
        Cal["Calendars<br/>(Azure DB-backed prod · Google dev)"]
    end

    Prospect ==>|inbound| Channels
    Channels ==>|outbound| Prospect

    Manager ==>|HTTPS + JWT bearer| SPA
    SPA -.->|EventSource SSE<br/>?token=| MgrAPI
    BE ==>|httpx + X-Manager-Token| MgrAPI
    ...
    Loop -->|"raw SQL +<br/>Postgres↔T-SQL translator"| DB
    Async --> DB
    BE -->|SQLAlchemy + pyodbc| DB
    ...
    class Channels,Entry,Loop,Retrieval,Async,MgrAPI agent
    class SPA,BE web
    class DB db
    class Anthropic,OpenAI,Tavily,Twilio,MSGraph,FBGraph,Cal ext
```

The diagram shows the three pieces as parallel subsystems sharing a single Azure SQL database. There are three distinct integration seams between them: (1) browser to KVR backend over HTTPS+JWT for direct DB-backed reads and writes, (2) browser to agent service over HTTPS+SSE for live updates, and (3) KVR backend to agent service over HTTP for proxied manager actions. External services are agent-side only — the web platform talks to no third parties beyond Azure SQL itself.
