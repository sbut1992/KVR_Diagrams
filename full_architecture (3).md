...
```
  ...
  ─────────────────────────────────────────────────────────────────────────
    ...
    Agent     ──►  SPA         SSE event "ticket_created"
  ...
  ─────────────────────────────────────────────────────────────────────────
    ...
    KVR       ──►  SPA         ticket list
    SPA       ──►  Manager     render in Tasks page

  PHASE 3 — Manager submits an answer
  ─────────────────────────────────────────────────────────────────────────
    Manager   ──►  SPA         type answer + submit
    SPA       ──►  KVR         POST /api/tickets/{id}/answer
    KVR       ──►  Agent       POST /manager/tickets/{id}/answer
    Agent     ──►  DB          UPDATE ticket · persist manager reply

  PHASE 4 — Async delivery back to the prospect
  ─────────────────────────────────────────────────────────────────────────
    APScheduler 60s job dispatches the answer per channel:
      • WhatsApp / SMS:  deliver once prospect has been idle ≥ 15 min
      • Email:           deliver immediately on manager answer
      • Web:             SPA polling picks up the response
